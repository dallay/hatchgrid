...
import { LoggerConfigurationBuilder,LogLevel, ConsoleTransport } from '../src';

describe('LoggerConfigurationBuilder', () => {
  it('should start with development defaults', () => {
    const config = LoggerConfigurationBuilder.development().withTransport(new ConsoleTransport()).build();
    expect(config.level).toBe(LogLevel.DEBUG);
  });

  it('should start with production defaults', () => {
    const config = LoggerConfigurationBuilder.production().withTransport(new ConsoleTransport()).build();
    expect(config.level).toBe(LogLevel.INFO);
  });

  it('should set a custom default log level', () => {
    const config = new LoggerConfigurationBuilder().withTransport(new ConsoleTransport()).build();
    expect(config.level).toBe(LogLevel.INFO);
  });

  it('should add a transport', () => {
    const transport = new ConsoleTransport();
    const config = LoggerConfigurationBuilder.development().withTransport(transport).build();
  ...
  it('should set multiple logger-specific levels at once', () => {
    ...
    const config = LoggerConfigurationBuilder.development().withLoggerLevels(levels).withTransport(new ConsoleTransport()).build();
