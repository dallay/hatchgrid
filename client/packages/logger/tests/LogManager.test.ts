...
import { LogManager,  Logger , LoggerName, LogLevel} from '../src';
import type { LoggerConfiguration, LogEntry, Transport } from '../src';
...
describe('LogManager', () => {
  ...
  describe('configure', () => {
    ...
    it('should clear level cache when configured', () => {
      ...
      expect(LogManager.isLevelEnabled(cacheLogger, LogLevel.TRACE)).toBe(false);
    });
    ...
    it('should register LogManager globally', () => {
      ...
      expect((globalThis as Record<string, unknown>).__LOGGER_MANAGER__).toBe(LogManager);
    });
  ...
  describe('isLevelEnabled', () => {
    ...
    it('should return false if LogManager is not configured', () => {
      LogManager.reset();
      expect(LogManager.isLevelEnabled('test' as LoggerName, LogLevel.INFO)).toBe(false);
    });
    ...
    it('should respect specific logger levels', () => {
      ...
      expect(LogManager.isLevelEnabled(uiComponents, LogLevel.INFO)).toBe(false); // Below ui.components' WARN
    });
    ...
    describe('enhanced error handling for isLevelEnabled', () => {
      ...
      it('should return false for null loggerName', () => {
        expect(LogManager.isLevelEnabled(null as any, LogLevel.INFO)).toBe(false);
      });

      it('should return false for undefined loggerName', () => {
        expect(LogManager.isLevelEnabled(undefined as any, LogLevel.INFO)).toBe(false);
      });

      it('should return false for empty string loggerName', () => {
        expect(LogManager.isLevelEnabled('' as LoggerName, LogLevel.INFO)).toBe(false);
      });
      ...
      it('should return false for non-string loggerName', () => {
        expect(LogManager.isLevelEnabled(123 as any, LogLevel.INFO)).toBe(false);
      });

      it('should return false for invalid level (not a number)', () => {
        expect(LogManager.isLevelEnabled('test' as LoggerName, 'invalid' as any)).toBe(false);
      });

      it('should return false for invalid level (negative number)', () => {
        expect(LogManager.isLevelEnabled('test' as LoggerName, -1 as LogLevel)).toBe(false);
      });
    });
  });
  ...
  describe('clearCaches', () => {
    it('should clear the level cache', () => {
      ...
      expect(LogManager.isLevelEnabled(cacheLogger, LogLevel.DEBUG)).toBe(false);
    });
  });
  ...
  describe('reset', () => {
    it('should reset all LogManager state', () => {
      ...
      expect((globalThis as Record<string, unknown>).__LOGGER_MANAGER__).toBeUndefined();
    });
  });
});
