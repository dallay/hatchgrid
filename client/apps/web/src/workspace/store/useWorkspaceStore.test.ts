...
import type { WorkspaceRepository } from "@/workspace/domain/repositories";
import { createWorkspaceStore, type WorkspaceStoreDependencies, type WorkspaceUseCases } from "./useWorkspaceStore";
...
const createMockUseCases = (): WorkspaceUseCases => ({
  listWorkspaces: {
    ...
    execute: vi.fn<[], Promise<CollectionResponse<Workspace>>>(),
  },
  getWorkspaceById: {
    repository: {} as WorkspaceRepository,
    execute: vi.fn<[string], Promise<SingleItemResponse<Workspace> | null>>(),
  },
});
  ...
  describe("Caching", () => {
    describe("Workspace List Caching", () => {
      it("should cache workspace list after successful load", async () => {
        const mockExecute = mockUseCases.listWorkspaces.execute as MockedFunction<
    ...
    describe("Workspace Details Caching", () => {
      it("should cache workspace details after successful selection", async () => {
        const mockExecute = mockUseCases.getWorkspaceById.execute as MockedFunction<
      ...
      it("should invalidate multiple workspace details caches", async () => {
        ...
        store.invalidateCache({ workspaceDetails: [mockWorkspaces[0].id, mockWorkspaces[1].id] });
    ...
    describe("Cache Management", () => {
      it("should invalidate all caches", async () => {
        const mockSelectExecute = mockUseCases.getWorkspaceById.execute as MockedFunction<
          (id: string) => Promise<SingleItemResponse<Workspace> | null>
        >;
