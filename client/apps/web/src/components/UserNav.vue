...
const user = computed(() => props.user ?? authStore.account ?? ({} as Account));
...
<template>
  <DropdownMenu>
    <DropdownMenuTrigger :as-child="true">
      ...
      >
        <Avatar class="h-8 w-8" :class="variant === 'full' ? 'rounded-lg' : ''">
          <AvatarImage :src="user.avatar || user.imageUrl || ''" :alt="user.name || user.firstName || ''" />
    ...
    <DropdownMenuContent
      :class="variant === 'full' ? 'w-[--reka-dropdown-menu-trigger-width] min-w-56 rounded-lg' : 'w-56'"
