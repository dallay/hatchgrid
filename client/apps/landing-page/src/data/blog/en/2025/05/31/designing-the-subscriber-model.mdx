...
We use PostgreSQL with enums for `subscriber_status` (ENABLED, DISABLED, BLOCKLISTED) and RLS policies based on `current_setting('lyra.current_organization')` to isolate each creator’s data.
